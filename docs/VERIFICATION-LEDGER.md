...
| NeoPixel WS2812B timing | ucsim: T0H=**362 ns** (250–550✓), T1H=**814 ns** (650–950✓), T0L=**814 ns** (700–1000✓), T1L=**452 ns** (300–600✓). All four windows pass. 72 bits (9 bytes), inter-byte gap 3074 ns, send 103.7 µs, pin LOW after → latch met. | **1 (lineage)** — **cross-checked** (`ucsim-stc 564825d`). `neo_v3.ihx` run through both `stc12_trace` (ucsim) and `emu_trace` (emu8051, post-`6cb9bc7`): all four timing windows pass on both, and an edge-by-edge comparison of **144 intervals** gives **max &#124;diff&#124; = 1 ns** (clock-period rounding), with **0 edges differing by more than 1 ns**. Runnable as `tests/rung_neopixel_cross.sh`. This row previously read "ucsim only, the emulators disagree on CLR/SETB/CPL" — that disagreement was a defect in emu8051, fixed in `6cb9bc7` and confirmed at 1 MC by ucsim `8350048`. Same lineage caveat as the other cross-emulator rows. | **A real WS2812B strip showing the correct colour.** The two-emulator agreement is now made; only silicon is above it. |
...
| SDCC native vs WASM byte-identity | Native SDCC 4.5.0 and the WASM four-stage pipeline emit **byte-identical firmware for 10 of 10 programs**, same origin, **no injected records** (`emu8051-stc` spec-update 009, widened in `ea918a3`, run 31516634687). Code paths: minimal, timer ISR, ADC polling, UART echo, switch/case jump table, unsigned long math, code-memory arrays, function pointers, multi-interrupt, xdata structs. 172–356 bytes each. The suite reports **counts, not a verdict** — how many match, how many differ, and whether a difference is the known sdld defect or something new. The comparison gate was itself verified: a deliberately corrupted image produces exit 1. | **2** — same source (SDCC 4.5.0) compiled two ways, so this checks the build and the pipeline, not the compiler's correctness. Not category 1: the cross-check is SDCC against itself. | Nothing here reaches silicon. Still **one memory model, one SDCC release, one target** — the ten programs retire "one source file", not those three. It would rise if a *different* toolchain produced the same image, which no second 8051 compiler in this project does. |
| AVR blink under avr8js | Compiled by the hosted avr-gcc endpoint, executed through bw-board's adapter: LED brightness **0.5882**, derived beforehand from VCC 5 V, R 220 Ω, LED Vf 2 V / Rd 10 Ω, pin Rth 25 Ω → I = 11.76 mA. Exact match. An earlier "close to 0.68" was wrong — it omitted the LED's dynamic resistance. | **1** — **cross-checked against simavr** (`bw-board spec-updates/avr-cross-check.md`). Two tests on identical hex (`.text` verified identical between plain and VCD-instrumented builds): a tight PB5 toggle — **21 transitions on both, all values agreeing**, first edge 813 vs 810 ns — and **the brightness firmware itself**, 5 transitions on both with an ON period of 500,000,125 vs 500,000,130 ns, a **5 ns** difference, under one 62.5 ns clock cycle. Both VCDs asserted non-empty first, so an empty trace cannot produce false agreement. **Cleaner independence than the 8051 rows**: emu8051 and ucsim were both modified in this campaign, whereas neither avr8js nor simavr was — only the adapters around them. The argument that this cross-checks the right half is stated in the spec-update: brightness is pin duty times a circuit model, and the circuit model is our own analytic derivation, never simulator-dependent. | **An ammeter on a real Nano.** No AVR silicon has run anything. A **third** implementation now agrees: `ucsim-stc 1b4c257` ran the same 6-word blink under `ucsim_avr` and hand-verified every instruction cost (SBI=2, DEC=1, BRNE-taken=2, LDI=1, RJMP=2), giving a 769-cycle toggle period identical to avr8js — an analytic anchor rather than a fourth simulator run. It also loaded and toggled correctly on hex from the AVR compile endpoint. Its own caveats are stated: word-0 clobber on hex load, wrong disassembler register names, AT90S-era IO naming — none touching cycle counts. |
